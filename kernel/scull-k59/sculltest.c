...
#include <stdio.h>
#include <fcntl.h>
#include "scull-ioctl.h" /* include header file specific defined for ioctl access for this driver */
#include <sys/ioctl.h>

int main() {
	int fd, result, len, quantum;
   char buf[10];
   const char *str;
   ...
   result = ioctl(fd, SCULL_IOCGQUANTUM, &quantum);
   if (result < 0)
	   fprintf(stdout, "failed: to get quantun value");
   else
	   fprintf(stdout, "successful to get quantum is %d \n", quantum);
   
   result = ioctl(fd, SCULL_IOCTASKLET, NULL);
   result = ioctl(fd, SCULL_IOCWORKQUEUE, NULL);
