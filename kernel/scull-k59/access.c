...
static int scull_u_open(struct inode *inode, struct file *filp) {
   ...
   if (scull_u_count && 
       (uid_eq(scull_u_owner, cred->uid) &&  /* allow user */
	uid_eq(scull_u_owner, cred->euid) && /* allow whoever did su */
...
static inline int scull_w_available(void) {
	...
	return scull_w_count == 0 ||
		uid_eq(scull_w_owner,cred->uid) ||
		uid_eq(scull_w_owner,cred->euid) ||
